...
const getDaysLabel = (design: any) => {
  ...
  return { text: `${r} ngày`, color: 'text-slate-500', bg: 'bg-slate-100' };
};
...
export default function TrashPanel() {
  ...
  const load = async () => {
    try { setLoading(true); const d = await fetchTrashDesigns(); setDesigns(d.designs || []); }
  ...
  const handlePermanentDelete = async () => {
    ...
    try { await permanentlyDeleteDesign(confirmDelete.id); setDesigns(p => p.filter(d => d.id !== confirmDelete.id)); setConfirmDelete(null); showToast('🗑️ Đã xóa vĩnh viễn'); }
    catch (e: any) { showToast(`❌ ${e.message}`); } finally { setActionLoading(null); }
  };
    ...
    <div className="w-full min-h-full" onMouseDown={handleMouseDown}>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="fixed top-6 right-8 z-50 bg-slate-800 text-white px-5 py-3 rounded-2xl shadow-xl font-bold text-sm">{toast}</motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-extrabold text-slate-800">Thùng Rác</h2>
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center gap-3 mb-6">
            <span className="px-3 py-1 bg-rose-100 text-rose-600 rounded-full text-xs font-bold">{designs.length} thiết kế</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 items-start content-start">
            {designs.map((design, idx) => {
              const dl = getDaysLabel(design);
                ...
                <motion.div key={design.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className={`absolute top-3 left-3 z-20 transition-opacity ${selectedIds.includes(design.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <input type="checkbox" checked={selectedIds.includes(design.id)}
                      onChange={e => { if (e.target.checked) setSelectedIds(p => [...p, design.id]); else setSelectedIds(p => p.filter(id => id !== design.id)); }}
                  ...
                  <div className="aspect-[4/3] bg-gradient-to-br from-slate-50 to-slate-100 relative flex items-center justify-center overflow-hidden border-b border-slate-100">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-3 gap-3">
                      ...
                      <button onClick={() => setConfirmDelete(design)} disabled={isActing}
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] font-bold ${dl.color} ${dl.bg} px-2 py-0.5 rounded-full`}>Còn {dl.text}</span>
                    </div>
                    <p className="text-[11px] text-slate-400">Xóa: {design.deleted_at ? new Date(design.deleted_at).toLocaleDateString('vi-VN') : '—'}</p>
                  </div>
                </motion.div>
              );
            })}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
              ...
              <div className="flex items-center gap-4 mb-5">
                <div className="w-12 h-12 bg-rose-100 rounded-2xl flex items-center justify-center shrink-0"><AlertTriangle size={22} className="text-rose-500" /></div>
                <div><h3 className="font-extrabold text-slate-800 text-lg">Xóa vĩnh viễn?</h3><p className="text-sm text-slate-500 mt-0.5">Hành động này không thể hoàn tác!</p></div>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setConfirmDelete(null)} className="flex-1 py-3 rounded-2xl border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition">Hủy</button>
                <button onClick={handlePermanentDelete} disabled={actionLoading === confirmDelete?.id}
                  className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-rose-500 to-red-600 text-white font-bold transition shadow-md disabled:opacity-60 flex items-center justify-center gap-2">
                  <Trash2 size={15} /> {actionLoading === confirmDelete?.id ? 'Đang xóa...' : 'Xóa vĩnh viễn'}
                </button>
              </div>
            </motion.div>
          </motion.div>
